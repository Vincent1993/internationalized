...
| 命令 | 说明 |
|------|------|
| `pnpm test -- --run` | 以无 watch 模式运行完整测试套件 |
| `pnpm test:coverage` | 生成覆盖率报告，定位薄弱测试点 |
| `pnpm typecheck` | 使用 TypeScript 进行静态类型检查，确保公共 API 类型安全 |

> 当前仓库仍存在少量历史遗留的类型告警，`pnpm typecheck` 会提示这些问题并以非零状态结束，便于后续逐步修复。

GitHub Actions (`.github/workflows/ci.yml`) 会在 push 与 Pull Request 触发同样的安装、类型检查与测试流程，保证远端仓库与本地环境行为一致。
