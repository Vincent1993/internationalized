...
[![Coverage](https://img.shields.io/badge/coverage-88%25-brightgreen.svg)](#-测试与质量)
[![Tests](https://img.shields.io/badge/tests-357%20passed-1E90FF.svg)](#-测试与质量)
...
项目使用 [pnpm](https://pnpm.io) 进行包管理，推荐按照以下步骤开始本地开发：

1. 安装依赖：`pnpm install`
2. 运行构建：`pnpm build`
3. 在 `tests/` 目录中添加或调整对应的单元测试/集成测试
4. 提交前执行质量检查（见下文）
...
| 检查项 | 命令 | 最近一次结果 |
|--------|------|---------------|
| 单元 & 集成测试 | `pnpm test -- --run` | ✅ 357 通过 / 0 失败（本地 Node 20 运行） |
| 覆盖率<sup>†</sup> | `pnpm test:coverage:ci` | ✅ Statements 87.97% · Branches 89.91% · Functions 82.71% · Lines 87.97% |
...
- 自定义扩展选项（如 `extend_fixDecimals`、`extend_negativeZero`）一并纳入哈希计算；
