...
/**
 * @name createNumberParser
 * @description 创建数字解析器实例，复用 `Intl.NumberFormat` 对本地化分隔符的解析能力。
 *
 * @since 0.0.1
 *
 * @param options - 解析配置，支持 locale、style、currency 等字段
 * @returns `NumberParser` 实例
 *
 * @example
 * ```ts
 * import { createNumberParser } from '@internationalized/number-format';
 *
 * const parser = createNumberParser({ style: 'currency', currency: 'CNY' });
...
/**
 * @name parseNumber
 * @description 快速解析字符串数值的辅助函数，相当于 `createNumberParser(options).parse(input)`。
 *
 * @since 0.0.1
 *
 * @param input - 待解析的字符串
 ...
 * @example
 * ```ts
 * import { parseNumber } from '@internationalized/number-format';
 *
 * const result = parseNumber('12.3%', { style: 'percent' });
